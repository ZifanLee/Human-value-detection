...
class BertFineTunerPl(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.validation_step_outputs.append([loss, outputs, labels])  # Append loss to validation_step_outputs list
    ...
    def on_validation_epoch_end(self):
        ...
        for output in self.validation_step_outputs:
            for i, out_labels in enumerate(output[2].detach().cpu()):
                labels.append(out_labels)
                predictions.append(output[1][:, i].detach().cpu())
    ...
    def on_train_epoch_end(self):
        ...
        for output in outputs:
            out_labels = output["labels"].detach().cpu()
            out_predictions = output["predictions"].detach().cpu()
            labels.append(out_labels)
