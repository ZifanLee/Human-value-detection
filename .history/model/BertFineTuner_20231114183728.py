...
class BertFineTuner(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        self.predictions = torch.sigmoid(output)  # Assign predictions to self.predictions attribute
...
def train(model, train_dataset, val_dataset, batch_size, num_epochs, learning_rate, n_warmup_steps, n_training_steps, save_path):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            optimizer.zero_grad()
            loss, outputs = model(input_ids, attention_mask, labels)  # Call the model's forward method
            loss.backward()
            optimizer.step()
            scheduler.step()
