...
class BertFineTunerPl(pl.LightningModule):
    ...
    def calculate_per_class_accuracy(self, logits, labels):
        predictions = logits.argmax(dim=1)
        correct_predictions = (predictions == labels).float()
        per_class_accuracies = []
        for class_index in range(self.num_classes):
            class_predictions = (predictions == class_index).float()
            class_labels = (labels == class_index).float()
            class_accuracy = (class_predictions * class_labels).sum() / class_labels.sum()
            per_class_accuracies.append(class_accuracy.item())
        return per_class_accuracies
